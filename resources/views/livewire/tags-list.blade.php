    ...
    @foreach($tags as $tag)
        <tr>
                ...
                <div class="flex items-center">
                    <div class="text-sm font-medium align-left text-gray-900
                        hover:text-green-600"><a
                            href="{{ route('content',['tag'=>$tag->id]) }}"> {{ $tag->name}}</a>
                    </div>
                </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">{{$tag->note->count()}}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                ...
                <a  href="{{route('tags.edit',[$tag->id])}}"  class="text-sm
                ...
                <button wire:click="$emit('deleteTag',{{$tag->id}})" class="text-sm text-red-600
                hover:text-red-400">
                    删除
                </button>
            </td>
